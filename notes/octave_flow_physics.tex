...
\author{为张潇文准备}
\date{2026 年 9 月 9 日（v3：审阅后修订，加真实 N-body 数据一节）}
...
%======================================================================
\section{真实 N-body 数据改变了什么}\label{sec:real}
%======================================================================

这一节是在第一批真实模拟上的 Phase 0 测量之后加的：生产数据 $z=0$ 的 $64^3/128^3/256^3$（$100\,h^{-1}$Mpc，16 个 seed，没有初条件），以及一对自己用 MP-Gadget 跑的 $32^3/64^3$（同一宇宙学，有初条件，$D(0)/D(99)=76.7$）。前面的代数一条都不用改；改的是微扰论那几节的适用范围、source 分布的定义、以及"新随机性"这个词的含义。

\subsection{量到了什么}

生产数据 $64\to128$（offset $\tfrac12$，立方体窗，16 个 seed 的散布 $0.7\%$）：修正 $\mathrm{rms}(\varepsilon)/h_c=0.27$（2LPT 玩具是 $0.001$--$0.004$）；$P_\varepsilon$ 在低 $k$ 是平的（斜率 $-0.16$，散度 $+1.7$），但比 $P_{\Psi_c}$ 低六个数量级，到 $k/k_{\Ny,c}=0.41,\,0.59,\,0.81,\,0.94$ 时 $P_\varepsilon/P_{\Psi_c}=0.03,\,0.13,\,0.37,\,0.57$；粗细互相关 $r_{cf}$ 在同样的 $k$ 是 $0.985,\,0.94,\,0.80,\,0.68$；Wiener 传递函数在 $1.07\,k_{\Ny,c}$ 降到 $0.5$；细节 $\Var(d/h_f)=0.25$，超额峰度 $3.2$，$\det(I+D^L)<0$ 的多流区占 Lagrangian 体积的 $37\%$。再下一级（$128\to256$）所有无量纲量都在漂：$0.39$、$0.47$、$6.0$、$42\%$。自跑的 $32\to64$（有初条件）：只用线性 octave，在 octave 带 $r=0.555$、$P_{\rm lin}/P_{\rm true}=1.49$，即只解释 $r^2=31\%$ 的细节方差而且功率超了；粗模式的 2LPT 谐波有细节功率的 $20\%$ 但和细节零相关（$r=-0.03$）；MP-GenIC 两级的初条件共享白噪声（低 $k$ 处 $r=1.00000$）但在粗 Nyquist 附近不是逐位嵌套的（$r=0.76$，功率比 $0.60$）。

\subsection{微扰论那几节的适用范围}

第 2--3 节描述的是 $\sigma(h_\ell,z)\lesssim1$ 的区间。$z=0$、$h_f\le1.6\,h^{-1}$Mpc 时 octave 带（$k\gtrsim1\,h/$Mpc）不在这个区间：线性 octave 是个差劲的预测，2LPT 在那里没用（谐波零相关就是微扰论失效，不是 bug）。结构性结论还在——粗场仍然通过 $D_{ij}$ 进入、octave 仍然是初条件层面唯一真正新的随机性、立方体分解仍然精确——但所有"修正很小""路径很短"的定量说法都要从数据重新来。由此三处修改。

\subsection{Source 改成最优线性预测}

线性理论只是从 $(\Psi_c,\eta)$ 到 $\Psi_f$ 猜测的一个特定线性映射。逐模式做最小二乘就得到最优的各向同性线性映射：
\begin{equation}
T_c(k)=\frac{P_{cf}(k)}{P_{cc}(k)}\ (\text{粗带}),\qquad G(k)=\frac{P_{d\times\rm lin}(k)}{P_{\rm lin}(k)}\ (\text{octave 带}),
\end{equation}
$P_{cf}$ 是插值粗场与细场的互功率，$P_{d\times\rm lin}$ 是细节与线性 octave 的互功率。修订后的 source 是
\begin{equation}
\boxed{\;x_0=P\big[T_c\,\Psi_c\big]+G\,D\,\frac{i\kk}{k^2}\hat\eta\;}
\label{eq:x0v3}
\end{equation}
在这一类 source 里 $\E\|x_1-x_0\|^2$ 最小（每个模式是一个独立的最小二乘问题，$T_c$、$G$ 就是回归系数）。微扰区里 $T_c,G\to1$，退回原来的 source。物理上 $G$ 是位移的 Lagrangian 传播子 $\langle\hat\Psi\hat\Psi^{(1)*}\rangle/P^{(1)}$，领头阶是 Lagrangian 重求和的高斯阻尼 $\exp(-k^2\sigma_\Psi^2/2)$（Matsubara 2008），这就是线性 octave 在 $z=0$ 超功率的原因；$T_c$ 是粗模拟的对应物——从一个在 Nyquist 附近已经错了的粗模拟出发对 $R\Psi_f$ 的线性估计。两者在训练集上测（按球壳平滑，$T_c$ 在 $0.3k_{\Ny,c}$ 以下钉为 1），都是确定性的，状态的信息量不变，第 5.2 节的定理原样适用。source 相对真值的 $1-r^2$（自跑数据 octave 带 $0.69$）是任何线性映射都给不出的那部分细节方差，是留给非线性模型的活。

\subsection{"嵌套初条件"改成"共享白噪声"}

第 2.4 节假设 $\hat\delta_c=W_{\rm cube}\hat\delta_f$。逐分辨率单独跑的生成器共享白噪声但在粗 Nyquist 附近离散化方式不同，$\chi':=W\hat\delta_f-\hat\delta_c\neq0$。$\chi'$ 给定 seed 是确定的，但不是 $(\Psi_c,\eta)$ 的函数；在混合式 \eqref{eq:mixture} 里它属于 $p(\delta_c\mid C)$ 的剩余不确定性，一阶时给修正贡献 $D\,i\kk\hat\chi'/k^2$。两个后果：nestedness check 在这类数据上不能当网格 offset 的探针（改从 $z_{\rm init}$ 的 snapshot 直接读）；emulator 模式下最干净的输入是整个细初条件而不只是它的 octave（Paper IV 的做法），这样 $\chi'$ 就从未知量里消失。生成模式下 $\chi'$ 仍是随机的，大小由初条件在 $k_{\Ny,c}$ 附近的 $1-r^2$ 界定。

\subsection{修正在 Nyquist 附近不小}

$k^2$ 定律在这批数据上看不见：低 $k$ 被离散性白噪底占据（无害，低六个数量级），$k_{\Ny,c}$ 附近粗模拟就是错的（$P_\varepsilon/P_{\Psi_c}=0.57$，$r_{cf}=0.68$）。于是修正拆成 Wiener 可预测的部分（由 source 里的 $T_c$ 提供）和一个相对方差为 $1-r_{cf}^2(k)$ 的残差（$0.94\,k_{\Ny,c}$ 处约 $0.54$）。原来"小、局域、近确定性的修正头"的图像换成：确定性的线性部分进 source，非线性残差和细节由同一个网络处理，两者都集中在粗带最上面三分之一。$r_{cf}$ 直到 $k_{\Ny,c}$ 都远大于零，这是不丢掉那些模式（立方体而非球）的定量理由。

\subsection{信息受限的条件分布}

第 4 节是精确的：给定 $(C,\eta)$ 且 $C$ 决定 $\delta_c$，$\Psi_f$ 就定了。但自跑数据里 $29\%$、生产数据里 $37$--$42\%$ 的 Lagrangian 体积是多流区，那里 $\eta\mapsto\Psi_f$ 是混沌的：它的 Jacobian 奇异值随坍缩后的轨道周期数指数增长。记 $\mathcal I_\epsilon$ 为以相对精度 $\epsilon$ 知道的 $(C,\eta)$（有限数据、有限网络、有限算术）。$\Var(\Psi_f\mid\mathcal I_\epsilon)$ 在单流区是 $O(\epsilon^2)$，在多流区对任何实际可达的 $\epsilon$ 都是 $O(1)$，因为所需精度随动力学时间指数下降。操作上：多流区比例就是第 5.5 节的回归落在 $P/P=r^2$ 线上、需要分布匹配目标的那部分体积；单流区是回归和 flow 应当一致的部分。这句话取代"精确算术下混沌不重要"：对精确的条件分布它不重要，对模型能表示的每一个条件分布它都重要。真实数据上回归 baseline 与 flow 的对比就是这一节的检验；预言是：flow 的优势（如果有）表现为 $r$ 低于 1 而 $P/P_{\rm true}$ 保持在 1，并且集中在多流的 Lagrangian patch 里。

\subsection{条件高斯性与普适性}

$z=0$ 的细节无条件峰度是 $3$--$6$；"给定局部 $D^L_{ij}$ 后近高斯"只能作为条件陈述，要从 Phase 0 面板 (c) 的分箱统计里读——多流 patch 里它会失败，那正是 flow 该起作用的地方。固定 $z$ 时各无量纲量逐级漂移 $40$--$90\%$，这是 style 变量 $s_\ell$ 必须吸收的量；跨层级共享权重至少要以 $\sigma(h_\ell,z)$ 和局部多流指示量为条件，而且它仍是一个待共享模型检验的假设。

\subsection{修订后的方法陈述}

立方体窗；source 用 \eqref{eq:x0v3}，$T_c,G$ 在训练集上测；物理配对；回归 baseline 和 flow 用相同输入并排训练；报告 octave 带的 $(r,P/P_{\rm true})$ 和粗带残差，各按粗场的多流指示量拆开；跨层级共享权重时以 $\sigma(h_\ell,z)$ 和局部 $D_{ij}$ 不变量为条件。第 2--3 节仍然是"为什么输入和分解是这样"的解释，但不再是工作区间的定量描述。
...
v3 的修改：第 \ref{sec:real} 节（真实 N-body 数据之后：微扰论的适用范围、数据驱动的 source、共享白噪声的初条件、Nyquist 附近的修正大小、信息受限的条件分布、修订后的方法陈述）。
